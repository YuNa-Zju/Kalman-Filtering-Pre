...
% --- 总览 ---
\begin{frame}
    \frametitle{算法总览：递归循环}
    卡尔曼滤波本质上是一个\textbf{“预测 (Predict) — 更新 (Update)”} 的递归过程。
    ...
    \begin{center}
        \begin{tikzpicture}[node distance=2cm, auto, >=latex]
            ...
            % 节点
            \node [block] (pred) {1. 时间更新 (预测)\\ \footnotesize 基于物理模型};
            \node [block, below of=pred] (gain) {2. 计算增益\\ \footnotesize 融合权重 $\bm{K}_k$};
            \node [block, below of=gain] (corr) {3. 校正\\ \footnotesize 基于观测数据};
        ...
        \end{tikzpicture}
    \end{center}
    % 如果不想用 tikz，可以直接用简化的 itemize，但上面的图示更直观
\end{frame}

% --- 第一阶段：预测 ---
\begin{frame}
    \frametitle{第一阶段：时间更新}
    在获取传感器数据前，基于上一时刻的状态与物理模型进行先验估计。
    ...
    \begin{block}{2. 协方差先验估计}
        ...
        \end{equation}
        误差随时间传递与扩散。$\bm{Q}$ 的引入表明预测过程本身增加了系统的不确定性。
    \end{block}
\end{frame}

% --- 核心：卡尔曼增益 (直觉) ---
\begin{frame}
    \frametitle{核心机制：卡尔曼增益}

    卡尔曼增益 $\bm{K}_k$ 是滤波器的灵魂，其本质是\textbf{最小均方误差准则下的最优加权因子}。
    ...
    \begin{itemize}
        \item 分子代表模型预测的误差方差。
        \item 分母代表系统总的误差方差。
        \item $\bm{K}_k$ 动态衡量了我们应该多大程度上信任“观测值”。
    \end{itemize}
\end{frame}
